package com.example.pruebaparcialmlkit;
...
import com.example.pruebaparcialmlkit.ml.ModeloBanderas;
import com.example.pruebaparcialmlkit.ml.ModeloBanderas;
        ...
        implements OnSuccessListener<Text>, OnFailureListener, ImageAnalysis.Analyzer, ImageReader.OnImageAvailableListener
{
    ...
    public void PersonalizedModel(View v) {
        //MODELO 
        try {
            ModeloBanderas model = ModeloBanderas.newInstance(getApplicationContext());
            TensorImage image = TensorImage.fromBitmap(mSelectedImage);

            ModeloBanderas.Outputs outputs = model.process(image);
    ...
    private void processImage() {
        ...
        try {
            ...
            String res="";
            for (int i = 0; i < probability.size(); i++)
                res = res + "Pais:  " + probability.get(i).getLabel() +  "\nPorentaje:  " +  probability.get(i).getScore()*100 + " % \n\n";
